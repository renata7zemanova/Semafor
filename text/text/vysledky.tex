...
%text o konkrétní m vybraném LoRa modulu
...
Pin EN slouží pro povolení funkce mikrokontroléru. Tento pin nesmí zůstat nezapojený, tzv. floating. Jeho zapojení je převzato z dokumentace, tj. pullup rezistor 10 k$\Omega$
a ke GND je připojen přes kondenzátor o hodnotě 1 $\mu$F \cite{ESP_C3_dtsh}.

%schéma zapojení a připojení všech periferií

ESP32-C3 má konfigurační piny, které slouží pro správné nastavení mikrokontroléru po restartu. Tyto piny musí být při restartu v daném nastavení. Konfiguračními piny jsou 
GPIO2, GPIO8 a GPIO9. Piny GPIO8 a GPIO9 nesmí být nikdy současně v logické nule. 
%napsat další info o strapping pins
...
\begin{table}[!h]
  ...
  \begin{center}
      ...
      \textbf{}	& \textbf{}	& \textbf{z SPI} & \textbf{z interní paměti} \\
	    \hline
	    \textbf{GPIO2}	& N/A & 1 & 1 \\ %co znamená N/A
	    \hline
	    \textbf{GPIO8}	& N/A & Nezáleží & 1 \\ %co znamená N/A
	    \hline
...
Při realizaci Semaforu byla tedy zvolena kombinace napájení pomocí baterií o pomocí powerbanky. Článek baterie LiFePO4 byl vybrán právě kvůli již zmíněným vynikajícím 
...
přes $I^2C$. Díky tomu budou využity pouze 2 GPIO piny mikrokontroléru ESP32-C3 a ne 5 GPIO pinů, které by byly zapotřebí při zapojení bez komunikace pro
sběrnici $I^2C$.
...
Vybraný mikrokontrolér ESP32-C3 umožňuje komunikaci přímo po USB protokolu a není díky tomu zapotřebí žádného převodníku pro komunikaci \cite{ESP_C3_dtsh}. %jmenuje se to USB protokol?
