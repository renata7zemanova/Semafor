...
struct led_t {
  led_t(const int COUNT, const int PIN) : leds(COUNT, PIN/*, LED_WS2812*/) {} //typ LEDek? jak nastavit a co nastavit?
...
void LED_light(led_t &LED, Colors COLOR, const int FIRST_LED, const int LAST_LED){ 
  for(int pos = FIRST_LED; pos <= LAST_LED; ++pos){
    LED.leds.setPixelColor(pos, colors(LED, COLOR));
  }
}

//upravit, aby nebylo blokujici - muzu se zeptat na barvu - getPixelColor
